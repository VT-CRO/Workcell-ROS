...
class shelfUpdate(Node):
    def __init__(self):
        super().__init__('shelfUpdate')
        ...
        # Create publisher
        self.publisher_ = self.create_publisher(Int32, 'shelfUpdate', 10)
...
def main(args=None):
    ...
    rclpy.spin_once(shelf_update)  # Spin only once, then shutdown ------------------------ spin_once
    shelf_update.destroy_node()
    rclpy.shutdown()
